...
const Connect = () => {
	return (
		<motion.section
			className="relative overflow-hidden bg-slate-200 dark:bg-slate-800 py-10 md:py-16 w-full max-w-[calc(100%-32px)] mx-4 xs:mx-auto rounded-3xl"
		...
		>
			...
			<Container className="flex flex-col lg:flex-row items-center justify-between gap-10 md:gap-20 px-4 xs:px-8 md:px-10 w-full">
				<div>
					<RevealWrapper>
						<Text
							as="h1"
				...
				<div className="grid grid-flow-col grid-rows-2 gap-2 sm:gap-4 items-center">
					{socialLinks.map((social, idx) => (
						<RevealWrapper
							...
							delay={idx * 0.1}
							className={cn(
								'transition-all hover:scale-110 duration-300 hover:rotate-2',
								(idx + 1) % 3 === 0 && 'row-span-2',
								idx % 2 === 0 && 'hover:-rotate-2',
							)}
						>
							<Link href={social.href} isExternal className="group/social">
								<div
									className="size-20 sm:size-24 lg:size-28 bg-slate-100 dark:bg-main/10 border-none rounded-3xl flex flex-col items-center justify-center gap-1"
									style={{
										backgroundColor: social.backgroundColor,
									}}
								>
									<span
										className="translate-y-3 group-hover/social:translate-y-0 transition-all duration-500"
										style={{
